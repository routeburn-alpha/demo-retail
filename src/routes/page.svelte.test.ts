...
describe('Routeburn storefront', () => {
  ...
  it('the hero tagline reads "Find your peak"', async () => {
    const screen = render(StorefrontPage, { data });
  ...
  it('page root container has a light purple background', async () => {
    const screen = render(StorefrontPage, { data });
    const pageRoot = screen.getByTestId('page-root');
    await expect.element(pageRoot).toHaveStyle(`background-color: rgb(240, 235, 255)`);
  });
